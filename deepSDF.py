...
def generate_sdf_samples(num_samples=10000, radius=1.0): 
    ...
    return torch.tensor(points, dtype=torch.float32), torch.tensor(sdf_values, dtype=torch.float32).unsqueeze(1) 
...
num_epochs = 100 
batch_size = 128 
latent_code = torch.zeros((batch_size, latent_dim))  # Представление фигуры в латентном пространстве
