...
class Camera:
    def __init__(self, position: torch.Tensor, width: int, height: int, device, dtype):
        ...
        y, x = torch.meshgrid(torch.linspace(-1, 1, self.height, device=device, dtype=dtype),
                          torch.linspace(-self.aspect_ratio, self.aspect_ratio, self.width, device=device, dtype=dtype),
                          indexing='ij')
        
        self.directions = torch.stack([x, y, torch.full_like(x, 1.0)], dim=-1)
        self.directions = torch.nn.functional.normalize(self.directions, dim=-1).reshape((self.width * self.height, 3))
